    ...
    <ul class="off-canvas-list" style="margin: 0 0.8rem;">
        ...
        @foreach ($shop_categories as $category)
            @if (!empty($category['name']))
                <li>
                    <a href="{{$category['mobile_link']}}" class="off-canvas-submenu-call">{{$category['name']}} <!--<span class="right chm-offcanvas-toggle-on"></span> --></a>
                </li>  
                @if (is_array($category['child']))
                    <ul class="off-canvas-submenu no-bullet">
                        @foreach($category['child'] as $child)
                            <li><a href="{{$child['mobile_link']}}">{{$child['name']}}</a></li>
                        @endforeach
                    </ul>
                @endif                 
            @endif
