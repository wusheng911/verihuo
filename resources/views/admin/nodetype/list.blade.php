...
@push('scripts')
    ...
    <script src="{{ asset('assets/admin/js/nodetype/nodetype.js') }}"> </script>
    <script>
    getListUrl = "{{ action('Admin\NodeTypeController@listJson') }}";
    delAdUrl = "{{ action('Admin\NodeTypeController@index') }}";
    </script>
@endpush


@section('content')
      		<button type="button" onClick="window.location.href='{{ action('Admin\NodeTypeController@create') }}';" class="btn btn-lg btn-success">新建节点类型</button>
