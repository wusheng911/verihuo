		...
		onShow : function()
		{
			...
			if ( fakeImage && fakeImage.data( 'cke-real-element-type' ) && fakeImage.data( 'cke-real-element-type' ) == 'video' )
			{
				...
				for ( var i = 0, length = sourceList.count() ; i < length ; i++ )
				{
				  ...
				  // if(rawsrc.indexOf(Chaohun_config_cdn_video) != -1){
				  //   rawsrc = rawsrc.replace(Chaohun_config_cdn_video, "");
				  // }
				  videos.push( {src : rawsrc, type: item.getAttribute( 'type' )} );
				}
		...
		onOk : function()
		{
			...
			for(var i=0; i<videos.length; i++)
			{
				...
				if ( !video || !video.src )
					continue;
			        var prefixUrl = "";
				// if(video.src.indexOf(Chaohun_config_cdn_video) != -1){
				//   prefixUrl = "";
				// } else {
				//   prefixUrl = Chaohun_config_cdn_video;
				// }
				innerHtml += '<cke:source src="' + prefixUrl + video.src + '" type="' + video.type + '" />';
