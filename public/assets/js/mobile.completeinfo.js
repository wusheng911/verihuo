...
var username_shorter = '用户名长度不能少于1个字符';
...
var msg_un_shorter = '帐号长度不能少于1个字符';
var msg_un_length = '帐号最长不能超过20个字符';
var msg_un_format = '请输入合法的手机号或邮箱作为您的登陆帐号';
var msg_un_registered = '帐号已经存在,将会绑定到此账号上。';
var msg_can_rg = '可以注册';
...
function mobile_show_alert(item_short_name, message) {
...
$(document).ready(function() {
    $("#register_form :input").blur(function() {
        ...
        if (submit_disabled) {
            mobile_show_alert(item, message);
        ...
        if (($(this).is("input[name=mobile_number]") || 
            ...
            $.get('/ajax/registeredby/' + value, {}, function(data, textStatus) {
                ...
                if ((item.length > 0) && ((data == item) || (data == 'phone'))) {
                    ...
                    mobile_show_info(item, message);
                    //document.forms['formCustomer'].elements['Submit'].disabled = 'disabled';
                } else {
                    mobile_hide_alert(item);
    ...
    $("input[name=zphone]").click(function() {
        ...
        if (!Utils.isTel(value)) {
          mobile_show_alert("mobile_number", mobile_phone_invalid);
          return;
        }
        ...
        $.post('/ajax/sendsms',
            ...
            function(data, textStatus){
                if (data.code==2){
                    RemainTime();
                    mobile_show_info('', '短信验证码已经发送到您的手机');
                }else{
                    if(result.msg){
                        mobile_show_alert('', result.msg);
                    }else{
                        mobile_show_alert('', '短信验证码发送失败');
                    }
                }              
            });        
    });
...
function mobile_register() {
    ...
    if (mobile_code.length == 0) {
        mobile_show_alert('', msg_mobilecode_blank);
        return false;
    } else if (is_alert_on()) {
        return false;
    } else {
